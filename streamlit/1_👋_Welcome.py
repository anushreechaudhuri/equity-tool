...
welcome_tab, faq_tab, viz_tab, report_tab = st.tabs(["👋 Welcome", "ℹ️ FAQ", "How-To: 🗺 Visualize", "How-To: 📈 Report"])
...
with welcome_tab:
    ...
    st.markdown("In [**🗺 Visualize**](%EF%B8%8F_Visualize#explore-a-national-map), the tool presents an interactive national map that allows you to explore areas of interest by zooming, filtering, and searching. After identifying areas that you’d like a closer look at, the [**📈 Report**](Report#generate-a-report-for-your-location) feature generates a detailed overview of every census tract and affordable housing property in the area you select.")

    st.markdown("Click on the other tabs on this page to learn more about the tool and how to use it! For information about data sources, check out the [**💡 Sources**](Sources#data-sources) section.")
...
with faq_tab:
    ...
    with st.expander("What does it mean to be a “Disadvantaged” census tract?"):
        st.markdown("According to the [Department of Energy (DOE) website](https://www.energy.gov/diversity/justice40-initiative), the DOE’s working definition of “Disadvantaged” is based on cumulative burden and includes data for thirty six (36) burden indicators collected at the census tract level. These burden indicators can be grouped across the following four categories (the numbers in parenthesis are the number of indicators in each category):")
    ...
    with st.expander("What about U.S. Tribal Lands and Territories?"):
        st.markdown("According to the [DOE website](https://www.energy.gov/diversity/justice40-initiative), federally recognized tribal lands and U.S. territories are categorized as Disadvantaged in accordance with White House Office of Management Budget’s Interim Guidance “common conditions” definition of community. Tribal lands are defined from census boundaries for American Indian/Alaska Native/Native Hawaiian lands (U.S. Census Bureau, 2021). U.S. territories include Puerto Rico, Guam, American Samoa, Virgin Islands, and Northern Marianas (U.S. Census Bureau, 2019). The tool covers all U.S. census tracts, including those located within Tribal Nations, to the extent that data is available.")
...
with viz_tab:
    ...
    st.markdown(" * To zoom, use the scroll bar or pinch the touchpad. Alternatively, use the “+” and “-” buttons in the bottom left to zoom in and out.")
    st.markdown(" * To search for a specific location, use the search bar in the bottom left. If searching for a city or county, you’ll get more accurate results by specifying the state.")
    ...
    st.markdown(" * Click on the small paint drop icon, which displays “Apply Auto Style” on hover, to color the map according to the indicator in the title of the widget.")
    st.markdown(" * Click the three small dots to “toggle” the widget, which is unexpanded by default. By toggling the widget, you can filter by category or histogram range to only view areas meeting the selected parameters on the map. The widget dynamically adjusts to display values representing only the area currently in the map view.")
